...
from portal.models import (
    DailyReportingModel, Activities, staffTbl, 
    PersonnelModel, PersonnelAssignmentModel, FarmdetailsTbl,
    Community, cocoaDistrict, projectTbl
)
...
def daily_reports(request):
    ...
    return render(request, 'portal/activity_reporting/daily_reports.html', context)

@csrf_exempt
@require_http_methods(["GET"])
def daily_report_list_api(request):
    """API endpoint for DataTables to get daily reports"""
    try:
        ...
        if not user.is_superuser:
            try:
                ...
                # If user is PO, show only their reports
                if user.groups.filter(name='Project Officer').exists():
                    queryset = queryset.filter(agent=staff)
                # If user is RA, show only reports they are assigned to
...
def daily_report_create(request):
    ...
    try:
        ...
        if ra_ids:
            ...
            report.ras.set(ras)
            # Update no_rehab_assistants count
...
def daily_report_detail(request, report_id):
    """Get details of a specific daily report"""
    try:
        report = DailyReportingModel.objects.select_related(
            'agent', 'main_activity', 'activity', 'farm', 
            'community', 'projectTbl_foreignkey', 'district'
        ...
        return JsonResponse({'success': False, 'message': 'Daily report not found'}, status=404)
    except Exception as e:
        return JsonResponse({'success': False, 'message': str(e)}, status=500)

@csrf_exempt
@require_http_methods(["POST"])
def daily_report_update(request, report_id):
    """Update an existing daily report"""
    try:
        report = DailyReportingModel.objects.get(id=report_id)
        data = json.loads(request.body)
        
        # Only allow updates if status is Pending (0)
...
@csrf_exempt
@require_http_methods(["POST"])
def daily_report_delete(request, report_id):
    """Delete a daily report (soft delete)"""
    try:
        report = DailyReportingModel.objects.get(id=report_id)
        data = json.loads(request.body)
...
@csrf_exempt
@require_http_methods(["POST"])
def daily_report_submit(request, report_id):
...
@csrf_exempt
@require_http_methods(["POST"])
def daily_report_approve(request, report_id):
...
@csrf_exempt
@require_http_methods(["POST"])
def daily_report_reject(request, report_id):
...
# ============== HELPER VIEWS FOR DAILY REPORTS ==============

@csrf_exempt
@require_http_methods(["GET"])
def get_farms_by_po(request):
    """Get farms assigned to a specific Project Officer"""
    try:
        po_id = request.GET.get('po_id')
        
        if po_id:
            # Get farms from daily reports of this PO
            farms = FarmdetailsTbl.objects.filter(
                dailyreportingmodel__agent_id=po_id
            ).distinct().values('id', 'farm_reference', 'farmername', 'farm_size')
        else:
            farms = FarmdetailsTbl.objects.filter(
                delete_field='no'
            ).values('id', 'farm_reference', 'farmername', 'farm_size')[:100]
        ...
        for farm in farms:
            data.append({
                ...
                'farm_size': float(farm['farm_size']) if farm['farm_size'] else 0
            })
...
@csrf_exempt
@require_http_methods(["GET"])
def get_ras_by_po(request):
    """Get Rehab Assistants assigned to a specific Project Officer"""
    try:
        po_id = request.GET.get('po_id')
        
        if po_id:
            # Get RAs from personnel assignments
            ras = PersonnelModel.objects.filter(
                personnelassignmentmodel__po_id=po_id,
                personnelassignmentmodel__status=1  # Active assignments
            ).distinct().values('id', 'first_name', 'surname', 'primary_phone_number', 'momo_number')
        else:
            ras = PersonnelModel.objects.all().values(
                'id', 'first_name', 'surname', 'primary_phone_number', 'momo_number'
            )[:100]
